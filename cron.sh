...
jq -r '.centers[] | select(any(.sessions[]; .min_age_limit == 18 and .available_capacity > 1)) | [.name, .sessions[].session_id, .sessions[].available_capacity, .sessions[].min_age_limit, .sessions[].date] | @csv' /tmp/out.json > /tmp/found.csv
if [ -s /tmp/found.csv ]
then
        echo "Slots found"
